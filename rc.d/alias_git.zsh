# for git alias
# Although there are many here, in reality, I don't use much.
...
alias ga="git add"
alias gaa="git add ."
alias gam="git add . && git commit -a -m  "
...
alias gl="git log --graph --format=format:'%C(bold blue)%h %C(bold green)(%cr) %C(bold white)%cn: %C(bold yellow)%d%C(reset) %C(white)%s%C(reset)' --abbrev-commit --date=relative && echo '\033[2A'"
...
#alias gswm='git switch master 2> /dev/null || git switch main'
